...
# Se estiver dentro de /scripts, o root do projeto é um nível acima
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"

# Carregar Bibliotecas Centralizadas (Padrão Resilient Automation)
...
cleanup() {
    ...
    if [ $exit_code -ne 0 ]; then
        echo -e "\n${RED}✗ Script interrompido com erro ($exit_code)${NC}"
    fi
}
trap cleanup EXIT
...
pre_flight_checks() {
    ...
    if ! command -v nginx &> /dev/null; then
        log_error "Nginx não encontrado. Por favor, instale o Nginx primeiro."
        exit 1
    fi

    # Verificar conectividade básica
    if ! curl -s --connect-timeout 5 https://8.8.8.8 > /dev/null 2>&1; then
        log_warning "Sem conectividade com a internet. A configuração do SSL pode falhar."
    fi

    # Verificar se as pastas do Nginx são graváveis
    if [ ! -w "$NGINX_SITES" ]; then
        log_error "Sem permissão de escrita em $NGINX_SITES"
        exit 1
    fi
}
...
╔═══════════════════════════════════════════════════════════╗
║                                                           ║
║        🌐 N8N STACK DOMAIN MANAGER 🌐                    ║
║                                                           ║
║        Gerenciador de Domínios (Nginx + SSL)             ║
║                                                           ║
╚═══════════════════════════════════════════════════════════╝
${NC}
...
create_nginx_config() {
    ...
    cat > "$config_file" <<EOF
# Configuração criada por Domain Manager (n8n Stack)
# Domínio: $domain
# Porta: $port
# Data: $(date)
...
list_domains() {
    ...
    for config in "$NGINX_SITES"/*; do
        # Pular se não for um arquivo, se for 'default' ou se for backup do Certbot
        if [[ ! -f "$config" ]] || [[ "$(basename "$config")" == "default" ]] || [[ "$config" == *".bak" ]]; then
            continue
        fi

        # Capturar o domínio (procurando pela linha que começa com server_name e ignorando variáveis)
        # O awk pega o segundo campo e tr remove o ponto e vírgula
        local domain=$(grep -E '^\s*server_name\s+[^$]' "$config" | head -n 1 | awk '{print $2}' | tr -d ';' || echo "")
        
        # Se não encontrou server_name na linha, tenta uma busca mais genérica mas foge de variáveis de redirecionamento
        if [ -z "$domain" ]; then
    ...
    if [ $count -eq 0 ]; then
        echo "Nenhum domínio configurado encontrado em $NGINX_SITES."
    else
        echo
